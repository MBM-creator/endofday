...
async function validateStageForOrg(
  ...
  if (orgError || !org) {
    ...
    console.error('[api/stages/[stageId]/labour] Org lookup failed:', { requestId, orgSlug, supabaseError: supabaseErr });
  ...
  if (stageError || !stage) {
    ...
    console.error('[api/stages/[stageId]/labour] Stage lookup failed:', { requestId, stageId, supabaseError: supabaseErr });
    const res = NextResponse.json(
  ...
  if (jobError || !job) {
    ...
    console.error('[api/stages/[stageId]/labour] Job not in org:', { requestId, stageId, supabaseError: supabaseErr });
    const res = NextResponse.json(
      { ok: false, requestId, message: 'Stage not found' },
      { status: 404 }
    );
...
function parseNonNegativeNumber(value: unknown, fieldName: string): { ok: true; value: number } | { ok: false; message: string } {
  ...
  if (n < 0) {
    return { ok: false, message: `${fieldName} must be >= 0` };
  }
...
export async function PATCH(
  ...
  let body: { crewCount?: unknown; hoursWorked?: unknown };
  try {
  ...
  const crewCountResult = parseNonNegativeNumber(body.crewCount, 'crewCount');
  if (!crewCountResult.ok) {
    return jsonError(crewCountResult.message, 400, requestId);
  }
  const crewCount = crewCountResult.value;

  const hoursWorkedResult = parseNonNegativeNumber(body.hoursWorked, 'hoursWorked');
  if (!hoursWorkedResult.ok) {
    return jsonError(hoursWorkedResult.message, 400, requestId);
  }
  const hoursWorked = hoursWorkedResult.value;

  const labourHours = crewCount * hoursWorked;
    ...
    .upsert(
      {
        ...
        crew_count: Math.round(crewCount),
  ...
  if (upsertError) {
    ...
    console.error('[api/stages/[stageId]/labour] Upsert failed:', { requestId, supabaseError: supabaseErr });
    return serverError(requestId, supabaseErr.code ?? 'LABOUR_UPSERT', 'Failed to save labour');
  }
