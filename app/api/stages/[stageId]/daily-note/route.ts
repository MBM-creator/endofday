...
async function validateStageForOrg(
  ...
  if (orgError || !org) {
    ...
    console.error('[api/stages/[stageId]/daily-note] Org lookup failed:', { requestId, orgSlug, supabaseError: supabaseErr });
  ...
  if (stageError || !stage) {
    ...
    console.error('[api/stages/[stageId]/daily-note] Stage lookup failed:', { requestId, stageId, supabaseError: supabaseErr });
    const res = NextResponse.json(
  ...
  if (jobError || !job) {
    ...
    console.error('[api/stages/[stageId]/daily-note] Job not in org:', { requestId, stageId, supabaseError: supabaseErr });
    const res = NextResponse.json(
      { ok: false, requestId, message: 'Stage not found' },
      { status: 404 }
    );
...
export async function PATCH(
  ...
  const toStore = content === '' ? null : content;
  ...
  if (updateError) {
    ...
    console.error('[api/stages/[stageId]/daily-note] PATCH failed:', { requestId, supabaseError: supabaseErr });
    return serverError(requestId, supabaseErr.code ?? 'DAILY_NOTE_UPDATE', 'Failed to save daily note');
  }

  const res = NextResponse.json({
    ok: true,
    dailyNote: toStore,
    dailyNoteUpdatedAt: new Date().toISOString(),
  });
  res.headers.set('x-request-id', requestId);
  return res;
}
