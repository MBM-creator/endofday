...
export default function IrrigationQaSupervisorPage() {
  ...
  const approvableSections = sectionStates.filter((section) => section.code !== 'supervisor_final_approval');
  const allCleared = approvableSections.length > 0 && approvableSections.every((section) => section.cleared);
      ...
      <div className="max-w-2xl mx-auto">
        ...
        {!loading && !run?.supervisor_final_approved_at && (
          <>
            ...
            <p className="text-sm text-gray-600 mt-1">Available when every applicable irrigation section is cleared and the run is still active.</p>
            ...
            {!allCleared && <p className="mt-2 text-xs text-amber-800">Not all irrigation sections are cleared yet.</p>}
          </>
        )}
      </div>
    </div>
